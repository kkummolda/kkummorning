...
> **Single Source of Truth (SoT)**: [`/prototype/PRD_SOT.md`](file:///c:/안티그래비티/prototype/PRD_SOT.md)  

---

## 📌 1. Single Source of Truth (SoT) 동기화 대원칙

본 프로젝트의 모든 요구사항 변경, 이슈 발생, 구현 상태 변경은 반드시 **`/prototype/PRD_SOT.md`**를 유일한 진실의 원천(Single Source of Truth)으로 삼습니다.

### 🚨 [MANDATORY RULE] PRD SoT 동시 갱신 규칙
> **모든 AI 에이전트 및 개발자는 아래 3가지 사건 발생 시 `/prototype/PRD_SOT.md`를 즉시 갱신해야 합니다:**
...
| 경로 | 역할 | 참조 우선순위 |
| :--- | :--- | :--- |
| **`/index.html`, `/auth.html`, `/app.js`, `/styles.css`** | 실제 서비스되는 고객용 꿈모닝 앱 | **1순위 (Production Customer App)** |
| **`/prototype/PRD_SOT.md`** | PRD 요구사항 & 이슈 트래킹 Single Source of Truth | **1순위 (PRD SoT)** |
| **`/prototype/TECHNICAL_SPEC.md`** | 베테랑 IT PM 엔지니어링 제어 명세서 | **2순위 (Technical Spec)** |
